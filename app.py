...
# --- 1. 網頁核心外觀配置 ---
st.set_page_config(page_title="美股雷達 V06.2", page_icon="🔮", layout="wide")
st.title("🔮 美股量化沙盒 V06.2 (法人級多因子雙向感知與產業分流版 - 多線程加速版)")
st.markdown("已實裝 **V06.2 四重法人防衛網**：**VIX恐慌感知**、**基本面護城河 (FCF/PE/營收)**、**財報避險盾**、**產業屬性標籤** 與 **雲端雙向自選股寫入** (⚡ 已導入 ThreadPoolExecutor 多線程平行處理)")
...
# --- 8. 網頁三分頁系統 ---
tab_summary, tab_manage, tab_debug = st.tabs(["📊 V06.2 美股多因子混合矩陣", "➕ 自選清單線上管理", "🔍 深度數據與視覺化面板"])
...
with tab_summary:
    ...
    if st.button("🚀 啟動 V06.2 美股全自動多因子掃描引擎 (⚡ 多線程平行加速)", use_container_width=True):
...
with tab_debug:
    ...
    if st.session_state.calculated:
        ...
        if db_key in st.session_state.detail_db:
            ...
            fig.update_layout(title=f"<b>{debug_ticker} - {debug_strat} V06.2 軌跡圖</b>", xaxis_title="日期", yaxis_title="價格 ($)", template="plotly_dark")
